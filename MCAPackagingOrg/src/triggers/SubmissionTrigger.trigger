...
if(trigger.isAfter && [select IsActivate__c from MCATriggerExecuter__mdt where DeveloperName = 'SubmissionRollUpTrigger'].IsActivate__c) {
    ...
    if(trigger.isInsert || trigger.isUpdate || trigger.isUnDelete){
        ...
        new list<RollupService.fieldDefinition> {
            new RollupService.fieldDefinition('COUNT','Id','McaApp__Number_of_Submission__c')
        };
        RollupService.rollUpTrigger(fieldDefinitions,trigger.new,'McaApp__Submission__c','McaApp__Opportunity__c','Opportunity','');
    }
    ...
    if(trigger.isDelete){
        ...
        RollupService.rollUpTrigger(fieldDefinitions,trigger.old,'McaApp__Submission__c','McaApp__Opportunity__c','Opportunity','');
    }
}
}