...
if(trigger.isAfter && [select IsActivate__c from MCATriggerExecuter__mdt where DeveloperName = 'MerchantPaymentRollUpTrigger'].IsActivate__c ) {
    ...
    if(trigger.isInsert || trigger.isUpdate || trigger.isUnDelete){        
        ...
        new list<RollupService.fieldDefinition> {
            new RollupService.fieldDefinition('SUM','Payment_Amt__c','Total_Merchant_Payment__c')
        };
        RollupService.rollUpTrigger(fieldDefinitions,trigger.new,'Merchant_Payment__c','Opportunity__c','Opportunity','');
    }
    ...
    //if record is Deleted fire the following trigger condition
        ...
        RollupService.rollUpTrigger(fieldDefinitions,trigger.old,'Merchant_Payment__c','Opportunity__c','Opportunity','');
    }
